...
public class WorldManager {
    ...
    public static boolean clearWorldReference(World world) {
        ...
        try {
            ...
            MyWorldsLite.plugin.log(Level.WARNING, "Exception while removing world reference for '" + worldname + "'!");
            ex.printStackTrace();
        }
    ...
    public static File getServerFolder() {
        if (serverfolder == null) serverfolder = MyWorldsLite.plugin.getDataFolder().getAbsoluteFile().getParentFile().getParentFile();
        return serverfolder;
    }
    ...
    public static WorldInfo getInfo(String worldname) {
        ...
        if (info != null && MyWorldsLite.calculateWorldSize) {
    ...
    public static World createWorld(String worldname, long seed) {
        ...
        MyWorldsLite.plugin.log(Level.INFO, msg.toString());
        ...
        if (cgen == null) {
            if (gen != null) {
                MyWorldsLite.plugin.log(Level.SEVERE, "World '" + worldname + "' could not be loaded because the chunk generator '" + gen + "' was not found!");
                return null;
            }
        }
        ...
        for (i = 0; i < retrycount + 1; i++) {
            try {
                ...
                MyWorldsLite.plugin.log(Level.WARNING, "World load issue: " + ex.getMessage());
        ...
        if (w == null) {
            MyWorldsLite.plugin.log(Level.WARNING, "Operation failed after " + i + " retries!");
        } else if (i == 1) {
            MyWorldsLite.plugin.log(Level.INFO, "Operation succeeded after 1 retry!");
        } else if (i > 0) {
            MyWorldsLite.plugin.log(Level.INFO, "Operation succeeded after " + i + " retries!");
        }
        return w;
    }
    ...
    public static int repairRegion(File chunkfile, File backupfolder) {
        MyWorldsLite.plugin.log(Level.INFO, "Performing repairs on region file: " + chunkfile.getName());
        ...
        try {
            ...
            for (int i = 0; i < locations.length; i++) {
                ...
                try {
                    ...
                    if (length > 4096 * size) {
                        editcount++;
                        locations[i] = 0;
                        MyWorldsLite.plugin.log(Level.WARNING, "Invalid length: " + length + " > 4096 * " + size);
                        //Invalid length
                        ...
                        if (version == 1) {
                                ...
                                stream = null;
                                //Unknown version
                                MyWorldsLite.plugin.log(Level.WARNING, "Unknown region version: " + version + " (we probably need an update here!)");
                        }
                        if (stream != null) {
                            ...
                            try {
                                ...
                                if (base == null) {
                                    editcount++;
                                    locations[i] = 0;
                                    MyWorldsLite.plugin.log(Level.WARNING, "Invalid tag compound at chunk " + chunkX + "/" + chunkZ);
                                    ...
                                    if (comp.hasKey("Level")) {
                                        ...
                                        if (xPos != chunkX || zPos != chunkZ) {
                                            MyWorldsLite.plugin.log(Level.WARNING, "Chunk [" + xPos + "/" + zPos + "] was stored at [" + chunkX + "/" + chunkZ + "], moving...");
                                ...
                                editcount++;
                                locations[i] = 0;
                                MyWorldsLite.plugin.log(Level.WARNING, "Stream  " + i);
                                ex.printStackTrace();
                            }
                            stream.close();
                        }
            ...
            if (editcount > 0) {
                if (backupfolder.mkdirs() && copy(chunkfile, backupfile)) {
                    ...
                    MyWorldsLite.plugin.log(Level.WARNING, "Failed to make a copy of the file, no changes are made.");
                    return -2;
                }
            }
            //Done.
