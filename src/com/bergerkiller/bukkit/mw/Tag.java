...
public class Tag {
    ...
    public String toString(int indent) {
        if (this.type != Type.TAG_End || this.value != null) {
            String rval = getIndent(indent) + getTypeString(this.type);
            if (this.name != null) rval += "(" + "\"" + this.name + "\")";
            if (this.type == Type.TAG_Byte_Array) {
                byte[] b = (byte[]) this.value;
                rval += ": [" + b.length + " bytes]\n";
            } else if (this.type == Type.TAG_List) {
                Tag[] subtags = (Tag[]) this.value;
                rval += ": " + subtags.length + " entries of type " + getTypeString(this.listType) + "\n";
                for (Tag st : subtags) {
                    rval += st.toString(indent + 1);
                }
                rval += getIndent(indent) + "}\n";
            } else if (this.type == Type.TAG_Compound) {
                Tag[] subtags = (Tag[]) this.value;
                rval += ": " + (subtags.length - 1) + " entries\n";
                rval += getIndent(indent) + "{\n";
                for (Tag st : subtags) {
                    rval += st.toString(indent + 1);
                }
