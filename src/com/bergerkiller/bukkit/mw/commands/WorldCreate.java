                        ...
                        for (int x = -keepdimension; x < keepdimension; x++) {
                            ...
                            for (int z = -keepdimension; z < keepdimension; z++) {
                                ...
                                if (first || (current + 2) == total) {
                                    ...
                                    t = new Task(MyWorldsLite.plugin) {
                                        public void run() {
                                        message(ChatColor.YELLOW + "Preparing spawn area (" + percent + "%)...");
                                        MyWorldsLite.plugin.log(Level.INFO, "Preparing spawn area (" + percent + "%)...");
                                        }
                                    };
                                    first = false;
                                }
                                if (++current == total) {
                                    t = new Task(MyWorldsLite.plugin) {
                                        public void run() {
                                            ...
                                            MyWorldsLite.plugin.log(Level.INFO, "World '"+ world.getName() + "' loaded.");
                                        }
                                    };
                                }
